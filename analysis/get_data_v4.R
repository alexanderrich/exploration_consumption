...
rs = dbSendQuery(con, "select * from exploration_consumption where status=3 and codeversion=\"4.0\"")
data = fetch(rs, n=-1)
...
exploredata <- trialdata %>% filter(trialType == "choice")
...
write.csv(exploredata, file="../data/choice_data_v4_0.csv")
write.csv(consumptiondata, file="../data/consumption_data_v4_0.csv")
...
write.csv(questiondata, file="../data/question_data_v4_0.csv")

data {
  ...
  vector[N] currentVal; //normalized current state of task
  vector[N] condition; //+1 for delayed, -1 for immediate
  vector[N] bis; //normalized bis score
}
...
model {
  ...
  b_bis ~ normal(0, 5);
  b_bis_delay ~ normal(0, 5);

  subj_intercept_raw ~ student_t(5, 0, 1); // replace this with "normal" to do non-robust regression
  subj_slope_raw ~ student_t(5, 0, 1); // replace this with "normal" to do non-robust regression

data {
  ...
  vector[N] enjdiff; //normalized enjoyment difference score
}
...
model {
  ...
  b_enjdiff ~ normal(0, 5);
  b_enjdiff_delay ~ normal(0, 5);

  subj_intercept_raw ~ student_t(5, 0, 1);
  subj_slope_raw ~ student_t(5, 0, 1);
  ...
  predictor =
    ...
    b_enjdiff * enjdiff +
    b_enjdiff_delay * enjdiff .* condition;
  p = 1 ./ (1 + exp(-predictor));
  y ~ bernoulli(p);
}
